...
void setup()  
{
  ...
  if (digitalRead(9)==0){
    flag_online=0;
    Serial.println("ARD1 офлайн."); //второй контролдер отключен
  }
  else{
    flag_online=1;
    Serial.println("ARD1 онлайн."); //второй контролдер отключен
  }
}
...
void loop() {
  if ((digitalRead(9)==0)&&(flag_online==1)){
    Serial.println("ARD1 вышел из чата."); //второй контролдер отключен
    flag_online=0;
    delay(200);
  }
  if ((digitalRead(9)==1)&&(flag_online==0)){
    Serial.println("ARD1 вернулся в чат."); //второй контролдер отключен
    flag_online=1;
    delay(200);
  }
